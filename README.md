...
1. Clone the repo:
   ...
   ```bash
   git clone https://github.com/noobvishal/telegram-mirror-bot
...
2. Start a new tmux session with `tmux new -s mirror-bot`, or connect to an existing session with `tmux a -t mirror-bot`. Running the bot inside tmux will let you disconnect from the server without terminating the bot. You can also use nohup instead.
